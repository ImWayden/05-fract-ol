...
/*   Updated: 2023/03/19 00:08:22 by wayden           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    t_color color;
    color.r = ((start_color & 0xFF0000) >> 16) 
    * (1 - percent) + ((end_color & 0xFF0000) >> 16) * percent;
    color.g = ((start_color & 0xFF00) >> 8) 
    * (1 - percent) + ((end_color & 0xFF00) >> 8) * percent;
    color.b = (start_color & 0xFF) 
    * (1 - percent) + (end_color & 0xFF) * percent;
    color.a = 0;
    return (color.hex);
}
...
static void inti_pal(t_palette *pals)
{
    ...
    pals[8] = (t_palette){V_DK_CYAN,DK_CYAN,DKR_CYAN,CYAN,LGR_CYAN,LG_CYAN};
    pals[9] = (t_palette)
    {V_DK_MAGENTA,DK_MAGENTA,DKR_MAGENTA,MAGENTA,LGR_MAGENTA,LG_MAGENTA};
    pals[10] = (t_palette)
    {V_DK_GREEN,DK_GREEN,DKR_GREEN,GREEN,LGR_GREEN,LG_GREEN};
    pals[11] = (t_palette)
    {V_DK_YELLOW,DK_YELLOW,DKR_YELLOW,YELLOW, LGR_YELLOW,LG_YELLOW};
    pals[12] = (t_palette)
    {V_DK_BLUE,DK_BLUE,DKR_BLUE,BLUE, LGR_BLUE,LG_BLUE};
}
